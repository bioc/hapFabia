...
\caption{The DNA segment marked in yellow decended from a founder to
...
FABIA implemented in the package \Rpackage{fabia}
...
Examples of haplotype clusters found by hapFabia are given in
Fig.~\ref{fig:example1} and Fig.~\ref{fig:example2}.

\begin{figure*}[p]
\includegraphics[width=0.9\textwidth]{./figures/example1}
\caption{Example of a haplotype cluster. The $y$-axis
...
identified by hapFabia in violet.   \label{fig:example1}}
\end{figure*}

\begin{figure*}[p]
\includegraphics[width=0.9\textwidth]{./figures/example8}
\caption{Another example of a haplotype cluster. See Fig.~\ref{fig:example1}
 for a description.\label{fig:example2}}
\end{figure*}
...
format, and then 3.\ split into segments. The following command line
...
\end{enumerate}
The command line tools for step 2.\ and 3.\ are provided by the package
\Rpackage{hapFabia} in \texttt{inst/commandline/arch/}.
However step 2.\ and 3.\ can be performed in \R as well (see below).
...
R> print("Average haplotye cluster length:")
R> print(anaRes$avhaploClusterLengthS)
R> print("Average number of individuals belonging to haplotye clusters:")
R> print(anaRes$avnoIndividS)
R> print("Average number of tagSNVs of haplotye clusters:")
R> print(anaRes$avnoTagSNVsS)
R> print("Average MAF of tagSNVs of haplotye clusters:")
R> print(anaRes$avnoFreqS)
R> print("Average MAF within the group of tagSNVs of haplotye clusters:")
R> print(anaRes$avnoGroupFreqS)
R> print("Average number of changes between major and minor allele frequency:")
R> print(anaRes$avnotagSNVChangeS)
R> print("Averge number of tagSNVs per individual of a haplotype cluster:")
R> print(anaRes$avnotagSNVsPerIndividualS)
R> print("Average number of individuals that have the minor allele of tagSNVs:")
...
Then \texttt{filename.vcf} is converted by \Rfunction{vcftoFABIA} to sparse matrix format
\texttt{filename\_mat.txt} where the annotation file
...
duplicates of haplotype custers which occur because the segments overlap.
Next the function \Rfunction{analyzeIBDs} analyzes the results where
...
Then we plot a specific haplotype cluster, in this case the first (\texttt{ibdC=1}),
...
hapRes <- hapFabia(fileName="dataSim1fabia",prefixPath="",
          upperBP=0.15,p=10,iter=1,haploClusterLength=10,pMAF=0.1)
@
...
The histogram bin size in number of SNVs is computed from ``IBDlength''
...
(IBD runs), and therefore set $t=2$.
...
A haplotype cluster is initialized by an IBD run between
two core individuals that are identical at $m$ or more minor alleles.
The number $m$ is computed as $m = \textrm{minSNPsFactor} \times
k_{\textrm{min}}$.
